...
async function captureTodoReport(url: string, framework: string) {
  ...
  await flow.startTimespan({ stepName: 'Navigate' });
  const nextPageLink = 'a[href="/dashboard"]';
  await page.waitForSelector(nextPageLink);
  await page.click(nextPageLink);
  await page.waitForSelector('table');
  // await flow.snapshot({ stepName: 'On next page' });
  const todoLink = 'a[href="/todo"]';
  ...
  await page.click(todoButtonSelector);
  await page.click(todoButtonSelector);
  // await flow.snapshot({ stepName: 'Added todos' });
